...
class DbService {
    ...
    private async init() {
        try {
            ...
            console.warn('[DB] Backend offline. Usando modo local.');
            this.data = JSON.parse(JSON.stringify(DEFAULT_DB_STATE));
        }
    ...
    async getSystemDashboardStats(): Promise<any> {
        ...
        await this.ensureReady();
        return {
            server: { cpuLoad: 0.1, memoryUsage: 20, totalMemoryGB: "4.00", uptimeSeconds: 100, platform: "linux" },
            system: {
    ...
    async getFeatures(): Promise<Feature[]> { return [{ key: 'ai_analysis', name: 'Análise IA', description: 'Gemini' }]; }
    ...
    async saveDbConfig(config: DbConfig, username: string) { await this.persistState(); }
    async getSystemPrompt(): Promise<string> { await this.ensureReady(); return this.data.systemPrompt; }
    async setSystemPrompt(prompt: string, user: string) { await this.ensureReady(); this.data.systemPrompt = prompt; await this.persistState(); }
    ...
    async checkAndIncrementQuota(userId: number, isAi: boolean) {
        ...
        return { allowed: true, usage: user.usage, limit: 1000 };
    }
    ...
    async executeServerCommand(c: string) { return { success: true, output: "Simulado no modo fácil" }; }
    ...
    async testConnection() {
        try {
            ...
            if (res.ok) return { status: 'Conectado', details: 'Docker Online' };
            return { status: 'Erro', details: res.statusText };
        } catch (e) {
            return { status: 'Desconectado', details: 'Rede' };
        }
    }
}

...
const ModuleInstaller: React.FC<{ onInstall: (file: File) => Promise<void>, showToast: (message: string, type: 'success' | 'error') => void; }> = ({ onInstall, showToast }) => {
            ...
            {file && (
                 <div className="mt-4 text-center">
                    ...
                    <button
                        ...
                        className="mt-4 w-full bg-brand-blue text-white font-bold py-2 px-6 rounded-lg hover:bg-blue-600 transition-colors disabled:bg-brand-accent disabled:cursor-not-allowed flex items-center justify-center"
                    >
                         {isInstalling ? (
                             <>
...
const ModuleManagement: React.FC<ModuleManagementProps> = ({ showToast }) => {
    ...
    const handleInstallModule = async (file: File) => {
        try {
            await dbService.installModule(file);
            showToast('Módulo instalado com sucesso! Ative-o na lista.', 'success');
            fetchModules();
        } catch (error) {
            if (error instanceof Error) {
                showToast(error.message, 'error');
    ...
    return (
        <div className="space-y-8">
            ...
            <ModuleInstaller onInstall={handleInstallModule} showToast={showToast} />
        </div>
    );
};

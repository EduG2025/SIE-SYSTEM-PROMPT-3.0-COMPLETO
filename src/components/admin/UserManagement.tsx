...
const UserForm: React.FC<{ user: User | null; availablePlans: UserPlan[]; onSave: (user: User) => void; onCancel: () => void; }> = ({ user, availablePlans, onSave, onCancel }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        // Se for user novo, passamos sem ID para o DB criar
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            <div>
                ...
                <input type="text" name="username" value={formData.username || ''} onChange={handleChange} className="w-full bg-brand-primary p-2 rounded border border-brand-accent focus:ring-brand-blue focus:border-brand-blue text-white" required />
            </div>
             <div>
                <label className="text-sm text-brand-light mb-1 block">Email</label>
                <input type="email" name="email" value={formData.email || ''} onChange={handleChange} className="w-full bg-brand-primary p-2 rounded border border-brand-accent focus:ring-brand-blue focus:border-brand-blue text-white" />
            </div>
             <div className="grid grid-cols-2 gap-4">
                <div>
                    <label className="text-sm text-brand-light mb-1 block">Role</label>
                    <select name="role" value={formData.role} onChange={handleChange} className="w-full bg-brand-primary p-2 rounded border border-brand-accent focus:ring-brand-blue focus:border-brand-blue text-white">
                 ...
                 <div>
                    ...
                    <select name="planId" value={formData.planId} onChange={handleChange} className="w-full bg-brand-primary p-2 rounded border border-brand-accent focus:ring-brand-blue focus:border-brand-blue text-white">
            ...
            <div className="grid grid-cols-2 gap-4">
                 <div>
                    ...
                    <input type="date" name="planExpiration" value={formData.planExpiration || ''} onChange={handleChange} className="w-full bg-brand-primary p-2 rounded border border-brand-accent focus:ring-brand-blue focus:border-brand-blue text-white" />
                </div>
                <div>
                    <label className="text-sm text-brand-light mb-1 block">Status</label>
                    <select name="status" value={formData.status} onChange={handleChange} className="w-full bg-brand-primary p-2 rounded border border-brand-accent focus:ring-brand-blue focus:border-brand-blue text-white">

...
const Header: React.FC<HeaderProps> = ({ modules }) => {
  ...
  return (
    <header className="bg-brand-secondary shadow-md flex justify-between items-center gap-4 px-4 md:px-6 py-3 shrink-0 border-b border-brand-accent/20">
      ...
      <div className="flex items-center gap-2 md:gap-4">
        ...
        <div className="relative hidden md:block">
          <input
            ...
            className="bg-brand-accent rounded-full py-1.5 pl-4 pr-8 w-40 md:w-64 text-sm text-brand-text placeholder-brand-light focus:outline-none focus:ring-2 focus:ring-brand-blue transition-all duration-300"
          />
           <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-brand-light absolute right-3 top-1/2 transform -translate-y-1/2 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
        </div>
        
        {activeModule?.hasSettings && !isSettingsPage && (
            <Link 
                to={`/${currentView}/settings`}
                className="p-2 rounded-full hover:bg-brand-accent transition-colors flex-shrink-0 text-brand-light hover:text-white"

...
const PoliticalModule: React.FC = () => {
    ...
    // Tratamento seguro de propriedades opcionais vindas do banco (JSON nulo)
    const safeRisks = politician.risks || { judicial: 'Baixo', financial: 'Baixo', media: 'Baixo' };
    const safeConnections = politician.connections || [];
    const safeVotingHistory = politician.votingHistory || [];
    const safeNews = politician.latestNews || [];
    const safeElectoralHistory = politician.electoralHistory || [];
    const safePartyHistory = politician.partyHistory || [];
    const safeDonations = politician.donations?.received || [];
    const safeAssets = politician.assets || { growthPercentage: 0, declarations: [] };
                    ...
                    <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
                        ...
                        <div className="flex-1">
                            <div className="flex items-center gap-3 mb-1">
                                ...
                                <span className={`px-3 py-0.5 rounded-full text-xs font-bold uppercase tracking-wider ${safeRisks.judicial === 'Crítico' ? 'bg-red-500/20 text-red-400' : 'bg-blue-500/20 text-blue-400'}`}>
                                    {safeRisks.judicial === 'Crítico' ? 'Alto Risco' : 'Em Análise'}
                                </span>
                            </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            ...
                            <div className="bg-brand-secondary border border-white/10 rounded-xl p-6">
                                ...
                                {safeVotingHistory.length > 0 ? (
                                    <div className="space-y-4 max-h-96 overflow-y-auto custom-scrollbar pr-2">
                                        {safeVotingHistory.map((vote, i) => (
                        ...
                        <div className="lg:col-span-1 space-y-6">
                            ...
                            <div className="bg-brand-secondary border border-white/10 rounded-xl p-6">
                                ...
                                <div className="space-y-3">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <RiskIndicator level={safeRisks.judicial} title="" />
                                    </div>
                                    <div className="flex justify-between items-center">
                                        <span className="text-sm text-brand-light">Financeiro</span>
                                        <RiskIndicator level={safeRisks.financial} title="" />
                                    </div>
                                    <div className="flex justify-between items-center">
                                        <span className="text-sm text-brand-light">Mídia/Imagem</span>
                                        <RiskIndicator level={safeRisks.media} title="" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-brand-secondary border border-white/10 rounded-xl p-6">
                                ...
                                <div className="space-y-2">
                                    {safeConnections.slice(0, 5).map((c, i) => (
                                        <div 
                                            key={i} 
                                    ...
                                    {safeConnections.length === 0 && (
                                        <p className="text-sm text-brand-light italic">Nenhuma conexão registrada.</p>
                                    )}
                                </div>
                            </div>
                        </div>
                ...
                {activeTab === 'financial' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="md:col-span-2 bg-brand-secondary border border-white/10 rounded-xl p-6">
                            ...
                            <DonationsReceived donations={safeDonations} />
                        </div>
                    </div>
                )}
                ...
                {activeTab === 'network' && (
                    <div className="h-[600px] bg-brand-secondary border border-white/10 rounded-xl overflow-hidden relative">
                         <Suspense fallback={<div className="w-full h-full flex items-center justify-center"><Spinner /></div>}>
                            <ConnectionGraph 
                                politician={{ ...politician, connections: safeConnections }} // Garante connections []

...
const getHighestRiskColor = (p: Politician): string => {
    // Check seguro para risks que podem ser nulos/undefined
    const r = p.risks || { judicial: 'Baixo', financial: 'Baixo' };
    if (r.judicial === 'Crítico' || r.financial === 'Crítico') return RISK_COLORS['Crítico'];
    if (r.judicial === 'Alto' || r.financial === 'Alto') return RISK_COLORS['Alto'];
    if (r.judicial === 'Médio' || r.financial === 'Médio') return RISK_COLORS['Médio'];
...
const NetworkGraph: React.FC<{ 
    ...
    const activeNodes = useMemo(() => {
        ...
        return nodes.filter(n => 
            ...
            (n.data.party && n.data.party.toLowerCase().includes(lowerTerm))
        );
    }, [nodes, searchTerm]);
                            ...
                            {selectedNode && (
                                <div className="w-80 bg-brand-secondary border border-brand-accent rounded-xl p-5 shadow-2xl flex flex-col animate-fade-in-up">
                                    ...
                                    <div className="grid grid-cols-2 gap-2 mb-4">
                                        <RiskIndicator level={selectedNode.risks?.judicial || 'Baixo'} title="Judicial" />
                                        <RiskIndicator level={selectedNode.risks?.financial || 'Baixo'} title="Financeiro" />
                                    </div>
                                ...
                                {currentPoliticians.map((pol) => (
                                    ...
                                    >
                                        ...
                                        <div className="px-6 relative flex-grow">
                                            ...
                                            <div className="grid grid-cols-2 gap-2 mb-4">
                                                <RiskIndicator level={pol.risks?.judicial || 'Baixo'} title="Judicial" />
                                                <RiskIndicator level={pol.risks?.financial || 'Baixo'} title="Financeiro" />
                                            </div>
                                        </div>

...
const ResearchModule: React.FC = () => {
    ...
    // Auto-search effect based on URL parameter ?autoQuery=...
    useEffect(() => {
        const searchParams = new URLSearchParams(location.search);
        const autoQuery = searchParams.get('autoQuery');
        
        if (autoQuery && autoQuery !== query) {
            setQuery(autoQuery);
            // Trigger search automatically if query is present
    ...
    const performSearch = async (searchQuery: string) => {
        ...
        try {
            const result = await performDeepInvestigation(searchQuery, filters, currentUser);
            setReport(result);
        } catch (error) {
            console.error(error);
        } finally {
